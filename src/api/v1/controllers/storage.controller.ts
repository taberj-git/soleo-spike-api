  ...
  uploadFileToStorage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    ...
    const MAX_FILE_SIZE = getServerConfig().maxFileSize as number;
      ...
      if (originalName !== req.file.originalname) {
        this.logger.warn(`Filename sanitized from ${req.file.originalname} to ${originalName}`);
      }
    ...
    try {
      ...
      this.logger.error('Exit StoreController.login caught an error:', err.message);
      next(err);  //push to global error handler
    }
